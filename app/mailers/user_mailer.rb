class UserMailer < ApplicationMailer
  ...
  def send_credentials_before_registration(user)
  	@user = user
  	mail(from: ENV['SENDER_EMAIL'], to: [ENV['ADMIN_1'], ENV['ADMIN_2']], subject: 'AdvertisingManagement - New Member Registration')
  end

  def send_credentials_before_password_reset(user)
  	@user = user
  	mail(from: ENV['SENDER_EMAIL'], to: [ENV['ADMIN_1'], ENV['ADMIN_2']], subject: 'AdvertisingManagement - Password Reset By Member')
  end
